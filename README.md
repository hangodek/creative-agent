...
* **🛡 Universal Polyglot Guardian (`rules/AGENTS.md`):** High-level architectural invariants (Organic User Flexibility, Anti-Binary Routing, Single Source of Truth / DRY constants, Component Line Budget < 250 lines, Autonomous Self-Verification Loop, 5 States of UI).
...
* **📐 The Canonical Embed Standard:** Zero design fragmentation. Banned generating truncated 30-line skeletons. Every UI project directly embeds the complete ~250–350 line brand specification from `skills/design-systems/design-<brand>/SKILL.md` into `design-system/MASTER.md`, preserving the full typography tracking table (down to `-3.0px`), 4px spacing ladder, elevation tiers, and brand anti-patterns.
* **👁️ Optical Fidelity & Component Type Floor:** Eliminates the "AI uncanny valley" and 12px ant-text syndrome. Enforces minimum type floors (action buttons 38px–44px height at `text-sm`/`text-base`, body copy 16px–17px, `text-xs` banned for primary content), proportional negative tracking (`tracking-[-0.03em]` on 32px+ headlines), strict brand weight discipline (e.g. Stripe *must* use Weight 300 Light headlines, never bold), OpenType `ss01` single-story glyphs and `tnum` tabular numerics, and curated open-source optical font substitutes for proprietary typefaces.
