...
* **👁️ Optical Fidelity Protocol:** Eliminates the "AI uncanny valley". Enforces proportional negative tracking (`tracking-[-0.03em]` on 32px+ headlines), strict brand weight discipline (e.g. Stripe *must* use Weight 300 Light headlines, never bold), OpenType `ss01` single-story glyphs and `tnum` tabular numerics, and curated open-source optical font substitutes for proprietary typefaces.
